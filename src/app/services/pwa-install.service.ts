...
export class PwaInstallService {
    ...
    getInstallInstructions(): { platform: string; instructions: string } {
        ...
        if (/iphone|ipad|ipod/.test(userAgent)) {
            return {
                platform: 'iOS',
                instructions: 'Tocca il pulsante Condividi e seleziona "Aggiungi a Home"'
            };
        } else if (/android/.test(userAgent)) {
            return {
                platform: 'Android',
                instructions: 'Tocca il menu e seleziona "Aggiungi a schermata Home" o "Installa app"'
            };
        } else {
            return {
                platform: 'Desktop',
                instructions: 'Cerca l\'icona di installazione nella barra degli indirizzi'
            };
        }
    }
}